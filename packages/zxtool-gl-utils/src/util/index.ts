...
export const createProgram = (gl: WebGLRenderingContext, vs: string, fs: string) => {
...
export const createVaoObj = (gl: WebGLRenderingContext | WebGL2RenderingContext) => {
...
export const getOrthographicMat = (l: number, r: number, t: number, b: number, n: number, f: number) => {
  return multiplyMats(
...
export const getPerspectiveMat = (fov: number, aspect: number, near: number, far: number) => {
  const [n, f] = [near, far]
